#!/bin/bash
# build_mac.sh - TEMseg macOS build (TIER 2)
#
...
# header). Unlike tier 1, tier 2 changes WHAT gets bundled, so run the API
# suite (scripts/mac/test_api.sh) against the resulting .app before
# distributing it.
#
...
for mod in torch torchvision numpy scipy sklearn skimage cv2 PIL tifffile \
           ...
           fastapi uvicorn pydantic webview PyInstaller; do
...
TARGET_APP="${DIST_DIR}/TEMseg-${SAFE_BRANCH}.app"
