...
# TEMseg PyInstaller spec - TIER 2 (macOS .app bundle, build-speed optimised)
#
# Tier 2 = every tier-1 change (see ../tier1/temseg_tier1.spec) plus removals
# that reduce build time and bundle size WITHOUT breaking runtime imports.
...
if _missing:
    print("[temseg_tier2.spec] FATAL. missing required input(s):", file=sys.stderr)
    for p in _missing:
...
print(f"[temseg_tier2.spec] Pruned {_pruned_count} data entries (~{_pruned_bytes // (1024*1024)}MB)")
