...
TIER="coreml"
SPEC_FILE="scripts/mac/coreml/temseg_coreml.spec"
DIST_DIR="dist/${TIER}"
WORK_DIR="build/${TIER}"
...
# takes <1s total.
#
# TIER2: gate covers the full runtime dependency set (not just the packages
# PyInstaller has trouble finding), so a broken venv fails here in <1s instead
# of several minutes into Analysis.
MISSING=""
for mod in torch torchvision numpy scipy sklearn skimage cv2 PIL tifffile \
           onnxruntime ultralytics segment_anything hyperspy rsciio \
           coremltools fastapi uvicorn pydantic webview PyInstaller; do
...
# STEP 4: Build executable with PyInstaller (tier-2 spec, tier-namespaced dirs)
...
RSCIIO_DEST="${DIST_DIR}/TEMseg-coreml.app/Contents/Frameworks/rsciio"
...
# TIER2: sanitise like the baseline script - replace every non-alphanumeric
# char with '-', strip leading/trailing dashes (tier1 only handled '/').
SAFE_BRANCH=$(printf '%s' "$BRANCH" | sed 's/[^a-zA-Z0-9]/-/g; s/^-//; s/-$//')
TARGET_APP="${DIST_DIR}/TEMseg-coreml-${SAFE_BRANCH}.app"
SOURCE_APP="${DIST_DIR}/TEMseg-coreml.app"
