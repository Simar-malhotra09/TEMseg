...
from app.api.instances import load_instances,extract_instances, save_instances, rasterize_instances, colorize_labeled_mask
...
async def get_instances(session_id: str):
    ...
    if not mask_path.exists():
        raise HTTPException(status_code=404, detail="No segmentation mask found — run segmentation first")

    logger.info(f"[MASKS] No cached instances, recomputing from mask.png")
...
async def api_save_instances(session_id: str, req: SaveInstancesRequest):
    logger.info(f"[MASKS] POST save | session={session_id} | {len(req.instances)} instances")
    session_dir = _session_dir(session_id)

    # we need image shape to rasterize — load from existing labeled mask or mask.png
    cached = load_instances(session_dir)
    if cached is not None:
        ...
        mask_path = session_dir / "mask.png"
        if not mask_path.exists():
            raise HTTPException(status_code=404, detail="No mask found to infer image shape")
        mask_bgr = cv.imread(str(mask_path))
        shape = mask_bgr.shape[:2]

    logger.info(f"[MASKS] Rasterizing into shape {shape}")

    # rebuild labeled mask from contours
    labeled = rasterize_instances(req.instances, shape)

    # save instances + labeled mask
    save_instances(session_dir, req.instances, labeled)

    # regenerate mask.png from new labeled mask
    ...
    return {"mask_url": f"/images/{session_id}/mask"}
...
async def split_instance(session_id: str, body: SplitRequest, request: Request):
    ...
    """
    t_start = time.time()
    logger.info(f"[SPLIT] POST split | session={session_id} | instance_id={body.instance_id} | {len(body.points)} points")
    ...
    cached = load_instances(session_dir)
    if cached is None:
        raise HTTPException(status_code=404, detail="No instances found — run segmentation and enter refine mode first")
    ...
    logger.info(f"[SPLIT] Target instance {body.instance_id} | area={target['area']} | bbox={target['bbox']}")
    ...
    logger.info(f"[SPLIT] SAM predictor state restored from cache")

    # ── 3. run SAM for each point ─────────────────────────────────────────────
    new_masks = []
    for i, (px, py) in enumerate(body.points):
        logger.info(f"[SPLIT] Running SAM predict | point {i+1}/{len(body.points)} | ({px:.1f}, {py:.1f})")
        t_pred = time.time()
        ...
        logger.info(f"[SPLIT] SAM predict done | point {i+1} | score={scores[best_idx]:.3f} | elapsed={time.time()-t_pred:.2f}s")

        # ── 4. constrain to original blob ────────────────────────────────────
        constrained = cv.bitwise_and(raw_mask, blob_mask)
        pixel_count = int(np.sum(constrained))
        logger.info(f"[SPLIT] Constrained mask | point {i+1} | pixels={pixel_count} (raw={int(np.sum(raw_mask))})")

        if pixel_count < 10:
            logger.warning(f"[SPLIT] Point {i+1} produced near-empty mask after constraint — skipping")
            continue

        new_masks.append(constrained)

    if not new_masks:
        raise HTTPException(status_code=400, detail="All split points produced empty masks — try placing points more centrally")

    logger.info(f"[SPLIT] Got {len(new_masks)} valid masks from {len(body.points)} points")
    ...
    for i, mask in enumerate(new_masks):
        ...
        logger.info(f"[SPLIT] New instance | id={new_id} | area={area} | contour_pts={len(contour)}")

    if not new_instances:
        raise HTTPException(status_code=400, detail="Could not extract valid contours from split masks")

    # ── 6. update instances list and labeled mask ─────────────────────────────
    updated_instances = [i for i in instances if i["id"] != body.instance_id] + new_instances
    logger.info(f"[SPLIT] Updated instance list | before={len(instances)} | after={len(updated_instances)}")
    ...
    mask_path = session_dir / "mask.png"
    cv.imwrite(str(mask_path), colored)
    logger.info(f"[SPLIT] Regenerated mask.png")

    t_total = time.time() - t_start
    logger.info(f"[SPLIT] Complete | {len(new_instances)} new instances | total={t_total:.2f}s")
